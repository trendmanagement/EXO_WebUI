...
conf = {}
if sys.argv[0].split(os.sep)[-1].lower() in ('fab.exe', "fab", "fab-script.py"):
...
def rsync_upload():
    ...
    excludes = ["*.pyc", "*.pyo", "*.db", ".DS_Store", ".coverage",
                "local_settings.py", "/.git", "/.hg", '/.idea', '__pycache__', '.thumbnails']
    local_dir = os.getcwd() + os.sep
    #
    #   Check windows PATH
    #
    if len(local_dir) > 3 and local_dir[1] == ':' and '\\' in local_dir:
        #We have windows path!
        local_dir = '/cygdrive/'+local_dir.replace(':','').replace('\\','/')

    return rsync_project(remote_dir=env.proj_path, local_dir=local_dir, extra_opts='--chmod=ug=rwX,o=rX -p',
                         exclude=excludes)
...
def install():
    ...
    apt("nginx libjpeg-dev python-dev python-setuptools git-core "
        "postgresql libpq-dev memcached supervisor python-pip python3-pip python3-setuptools python3-dev")
